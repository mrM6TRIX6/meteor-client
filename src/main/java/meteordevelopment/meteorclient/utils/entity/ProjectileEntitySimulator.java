...
public class ProjectileEntitySimulator {
        ...
        switch (item) {
            ...
            case CrossbowItem ignored -> {
                ...
                if (projectilesComponent.contains(Items.FIREWORK_ROCKET)) {
                    ...
                    set(user, angleOffset, accurate, tickDelta, ARROW.withPower(speed));
                }
    ...
    public boolean set(Entity entity) {
        ...
        if (entity instanceof ProjectileInGroundAccessor ppe && ppe.meteor$invokeIsInGround()) {
            return false;
        }
    ...
    /**
     * {@link ProjectileUtil#getCollision(Vec3d, Entity, java.util.function.Predicate, Vec3d, net.minecraft.world.World,
     * float, RaycastContext.ShapeType)}
     * <p>
     ...
     * simulated projectile may move from its start position through a block, only running the collision check
     * afterwards. The vanilla game has other code to deal with this but this is the easiest way for us to fix it.
     */
    private SimulationStep getCollision() {
        HitResult blockCollision = mc.world.getCollisionsIncludingWorldBorder(new RaycastContext(prevPos3d, pos3d, RaycastContext.ShapeType.COLLIDER, waterDrag == 0 ? RaycastContext.FluidHandling.ANY : RaycastContext.FluidHandling.NONE, simulatingEntity));
        ...
        if (simulatingEntity instanceof PersistentProjectileEntity) {
            Collection<EntityHitResult> entityCollisions = ProjectileUtil.collectPiercingCollisions(mc.world, simulatingEntity, prevPos3d, pos3d, dimensions.getBoxAt(prevPos3d).stretch(velocity.x, velocity.y, velocity.z).expand(1.0D), entity -> !entity.isSpectator() && entity.isAlive() && entity.canHit(), getToleranceMargin(), RaycastContext.ShapeType.COLLIDER, false);
            ...
            HitResult entityCollision = ProjectileUtil.getEntityCollision(mc.world, simulatingEntity, prevPos3d, pos3d, dimensions.getBoxAt(prevPos3d).stretch(velocity.x, velocity.y, velocity.z).expand(1.0D), entity -> !entity.isSpectator() && entity.isAlive() && entity.canHit(), getToleranceMargin());
            
            if (entityCollision == null || (age <= 1 && entityCollision instanceof EntityHitResult ehr && ehr.getEntity() == mc.player)) {
                return new SimulationStep(hitOrDeflect(blockCollision), blockCollision);
            }
            
            if (hitOrDeflect(entityCollision)) {
                return new SimulationStep(true, entityCollision);
            }
    ...
    /**
     * {@link ProjectileEntity#hitOrDeflect(HitResult)} {@link ProjectileEntity#onCollision(HitResult)}

...
/**
 * An extension of {@link net.minecraft.text.StringVisitable.StyledVisitor} with access to the underlying {@link Text}
 * objects.
 *
 * @param <T> the optional short circuit return type, to match the semantics of
 * {@link net.minecraft.text.StringVisitable.Visitor} and {@link net.minecraft.text.StringVisitable.StyledVisitor}.
 *
 * @author Crosby
 */
@FunctionalInterface
public interface TextVisitor<T> {
    ...
    private static void collectSiblings(Text text, Queue<Text> queue) {
        if (!(text.getContent() instanceof PlainTextContent ptc) || !ptc.string().isEmpty()) {
            queue.add(text);
        }

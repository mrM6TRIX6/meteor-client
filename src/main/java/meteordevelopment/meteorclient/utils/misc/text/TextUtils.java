...
public class TextUtils {
     ...
     *
     * @return You know what it returns. Read the docs! Also, returns white if the internal {@link Object2IntMap.Entry}
     * is null
     */
    ...
    /**
     * Takes a {@link List} of {@link ColoredText} and returns a {@link HashMap}, where the keys are all the existing
     * {@link Color}s in the aforementioned list, and the corresponding keys are the number of characters that have that
     * color.
     *
     * @param coloredTexts The list of {@link ColoredText} to obtain the color count of. Best paired with the output
     * from {@link #toColoredTextList(Text)}
     *
     * @return a {@link Map} whose keys are colors (and the set of keys being all possible colors used in the list, thus
     * all colors in the text, if the argument for this function is fed from the return from
     * {@link #toColoredTextList(Text)}), and the corresponding values being {@link Integer}s representing the number of
     * occurrences of text that bear that color. The order of the keys are in no particular order
     */
    ...
    /**
     * Performs a pre-order text traversal of {@link Text} components and ref-returns a sequential list of
     * {@link ColoredText}, such that one could know the text and its color by iterating over the list.
     *
     * @param text The text to flatten
     * @param stack An empty stack. This is used by the recursive algorithm to keep track of the parents of the current
     * iteration

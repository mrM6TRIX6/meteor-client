    ...
    private static void init() {
        ...
        MeteorExecutor.execute(() -> {
            for (Credit credit : credits) {
                ...
                request.exceptionHandler(e -> MeteorClient.LOGGER.error("Could not fetch repository information for addon '{}'.", credit.addon.name, e));
                ...
                switch (res.statusCode()) {
                    case Http.UNAUTHORIZED -> {
                        ...
                        MeteorClient.LOGGER.warn(message);
                        
                        if (System.getenv("meteor.github.authorization") == null) {
                            MeteorClient.LOGGER.info("Consider setting an authorization " +
                                "token with the 'meteor.github.authorization' environment variable.");
                            MeteorClient.LOGGER.info("See: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens");
                        }
                    }
                    case Http.FORBIDDEN ->
                        MeteorClient.LOGGER.warn("Could not fetch updates for addon '{}': Rate-limited by GitHub.", credit.addon.name);
                    case Http.NOT_FOUND ->
                        MeteorClient.LOGGER.warn("Could not fetch updates for addon '{}': GitHub repository '{}' not found.", credit.addon.name, repo.getOwnerName());
                    case Http.SUCCESS -> {

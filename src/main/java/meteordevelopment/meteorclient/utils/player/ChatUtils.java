...
public class ChatUtils {
    ...
    /**
     * Registers a custom prefix to be used when calling from a class in the specified package. When null is returned
     * from the supplier the default Meteor prefix is used.
     */
    @SuppressWarnings("unused")
    public static void registerCustomPrefix(String packageName, Supplier<Text> supplier) {
        for (Pair<String, Supplier<Text>> pair : CUSTOM_PREFIXES) {
        ...
        CUSTOM_PREFIXES.add(new Pair<>(packageName, supplier));
    }
    ...
    private static Text getPrefix() {
        ...
        for (Pair<String, Supplier<Text>> pair : CUSTOM_PREFIXES) {
            if (className.startsWith(pair.getLeft())) {
                ...
                return prefix != null ? prefix : ChatUtils.prefix;
            }
        }

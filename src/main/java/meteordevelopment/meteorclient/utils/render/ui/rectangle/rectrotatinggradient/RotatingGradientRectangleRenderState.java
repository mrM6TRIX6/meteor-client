...
final class RotatingGradientRectangleRenderState implements SimpleGuiElementRenderState {
    
    private final BuiltRotatingGradientRectangle built;
    private final Matrix3x2f pose;
    private final ScreenRect scissorArea;
    private final ScreenRect bounds;
    
    RotatingGradientRectangleRenderState(Matrix3x2f pose, BuiltRotatingGradientRectangle built, ScreenRect scissorArea) {
        this.built = built;
        this.pose = pose;
    ...
    public void setupVertices(VertexConsumer consumer) {
        int batchIndex = RotatingGradientRectangleRenderer.getInstance().reserve(built);
        if (batchIndex < 0) {
            return;
        }
    ...
    public RenderPipeline pipeline() {
        return RotatingGradientRectangleRenderer.ROTATING_GRADIENT_RECTANGLE_PIPELINE;
    }

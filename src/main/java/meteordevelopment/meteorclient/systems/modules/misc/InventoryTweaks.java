...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        if (!sortingEnabled.get()
            || !(mc.currentScreen instanceof HandledScreen<?> screen)
            || sorter != null
            || (mc.player.isCreative() && disableInCreative.get())
        ) {
            return false;
        }
        ...
        if (!mc.player.currentScreenHandler.getCursorStack().isEmpty()) {
            ...
            if (!empty.found()) {
                InventoryUtils.click().slot(-999);
            } else {
                InventoryUtils.click().slot(empty.slot());
            }
        }
    ...
    private void onTickPre(TickEvent.Pre event) {
        if (sorter != null && sorter.tick(sortingDelay.get())) {
            sorter = null;
        }
    }
    ...
    private void onTick(TickEvent.Post event) {
        // Auto Drop
        if (!Utils.canUpdate() || mc.currentScreen instanceof HandledScreen<?> || autoDropItems.get().isEmpty()) {
            return;
        }
        ...
        for (int i = autoDropExcludeHotbar.get() ? 9 : 0; i < mc.player.getInventory().size(); i++) {
            ...
            if (autoDropItems.get().contains(itemStack.getItem())) {
                if ((!autoDropOnlyFullStacks.get() || itemStack.getCount() == itemStack.getMaxCount())
                    && !(autoDropExcludeEquipped.get() && SlotUtils.isArmor(i))
                ) {
    ...
    @EventHandler
    private void onDropItems(DropItemEvent event) {
        if (antiDropOverrideBind.get().isPressed()) {
            return;
        }
    ...
    private void onInteractEntity(InteractEntityEvent event) {
        if (!antiItemFrame.get() || antiDropOverrideBind.get().isPressed()) {
            return;
        }
        if (!(event.entity instanceof ItemFrameEntity)) {
            return;
        }
        
        Item item = mc.player.getStackInHand(event.hand).getItem();
        if (antiDropItems.get().contains(item)) {
            event.cancel();
        }
    }
    ...
    private void onPacketSend(PacketEvent.Send event) {
        if (!xCarry.get() || !(event.packet instanceof CloseHandledScreenC2SPacket packet)) {
            return;
        }
        ...
        for (int i = start; i < end; i++) {
            ...
            if (steal && stealDrop.get()) {
                if (dropBackwards.get()) {
                    int iCopy = i;
                    Rotations.rotate(mc.player.getYaw() - 180, mc.player.getPitch(), () -> InventoryUtils.drop().slotId(iCopy));
                }
            } else {
                InventoryUtils.shiftClick().slotId(i);
            }
        }
    }

...
public class ReverseStep extends Module {
    ...
    private boolean isOnBed() {
        ...
        if (xa >= 0 && xa <= 0.3 && check(blockPos, -1, 0)) {
            return true;
        }
        if (xa >= 0.7 && check(blockPos, 1, 0)) {
            return true;
        }
        if (za >= 0 && za <= 0.3 && check(blockPos, 0, -1)) {
            return true;
        }
        if (za >= 0.7 && check(blockPos, 0, 1)) {
            return true;
        }
        
        if (xa >= 0 && xa <= 0.3 && za >= 0 && za <= 0.3 && check(blockPos, -1, -1)) {
            return true;
        }
        if (xa >= 0 && xa <= 0.3 && za >= 0.7 && check(blockPos, -1, 1)) {
            return true;
        }
        if (xa >= 0.7 && za >= 0 && za <= 0.3 && check(blockPos, 1, -1)) {
            return true;
        }

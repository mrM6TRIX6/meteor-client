...
public class Jesus extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (mc.player.isTouchingWater() && !waterShouldBeSolid()) {
            return;
        }
        ...
        if (tickTimer == 0) {
            ((IVec3d) mc.player.getVelocity()).meteor$setY(0.30);
    ...
    private boolean waterShouldBeSolid() {
        if (EntityUtils.getGameMode(mc.player) == GameMode.SPECTATOR || mc.player.getAbilities().flying) {
            return false;
        }
        ...
        if (dipIfBurning.get() && mc.player.isOnFire()) {
            return false;
        }
        
        if (dipOnSneakWater.get() && mc.options.sneakKey.isPressed()) {
            return false;
        }
        if (dipOnFallWater.get() && mc.player.fallDistance > dipFallHeightWater.get()) {
            return false;
        }
    ...
    private boolean lavaShouldBeSolid() {
        ...
        if (dipOnSneakLava.get() && mc.options.sneakKey.isPressed()) {
            return false;
        }
        if (dipOnFallLava.get() && mc.player.fallDistance > dipFallHeightLava.get()) {
            return false;
        }

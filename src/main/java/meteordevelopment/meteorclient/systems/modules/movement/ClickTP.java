...
public class ClickTP extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        if (mc.player.getInventory().getSelectedStack().getUseAction() != UseAction.NONE) {
            return;
        }
        ...
        if (mc.crosshairTarget != null) {
            if (mc.crosshairTarget.getType() == HitResult.Type.ENTITY && mc.player.interact(((EntityHitResult) mc.crosshairTarget).getEntity(), Hand.MAIN_HAND) != ActionResult.PASS) {
                return;
            }
            if (mc.crosshairTarget.getType() == HitResult.Type.BLOCK && mc.player.getMainHandStack().getItem() instanceof BlockItem) {
                return;
            }
        }
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (mc.world.getBlockState(pos).onUse(mc.world, mc.player, hitResult) != ActionResult.PASS) {
                return;
            }

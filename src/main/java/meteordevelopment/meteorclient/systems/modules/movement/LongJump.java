    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        switch (jumpMode.get()) {
            ...
            case BURST -> {
                ...
                if (onlyOnGround.get() && !mc.player.isOnGround() && stage == 0) {
                    return;
                }
                ...
                if (PlayerUtils.isMoving() && (!onJump.get() || mc.options.jumpKey.isPressed()) && !mc.player.isInLava() && !mc.player.isTouchingWater()) {
                    if (stage == 0) {
                        moveSpeed = getMoveSpeed() * burstInitialSpeed.get();
                    } else if (stage == 1) {
        ...
        if (Utils.canUpdate() && jumpMode.get() == JumpMode.GLIDE) {
            ...
            if (!mc.player.verticalCollision && !mc.player.isOnGround()) {
                ...
                if (velocityY < -0.2 && velocityY > -0.24) {
                    updateY(velocityY * 0.7 * glideMultiplier.get());
                } else if (velocityY < -0.25 && velocityY > -0.32) {
                    updateY(velocityY * 0.8 * glideMultiplier.get());
                } else if (velocityY < -0.35 && velocityY > -0.8) {
                    updateY(velocityY * 0.98 * glideMultiplier.get());
                }

...
import meteordevelopment.meteorclient.config.types.Vector3dSetting;
...
public class AutoWasp extends Module {
    ...
    public void onActivate() {
        if (target == null || target.isRemoved()) {
            ...
            if (target == null) {
                ...
                info(target.getName().getString() + " set as target.");
            }
        }
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (predictMovement.get()) {
            targetPos.add(PlayerEntity.adjustMovementForCollisions(target, target.getVelocity(),
        ...
        if (Math.abs(yDist) > 1.0E-5F) {
            if (Math.abs(yDist) < verticalSpeed.get()) {
                yVel = yDist;
            } else {

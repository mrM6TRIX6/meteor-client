...
public class ElytraFlyMode {
    ...
    public void onTick() {
        if (elytraFly.autoReplenish.get()) {
            FindItemResult fireworks = InventoryUtils.find(Items.FIREWORK_ROCKET);
            
            if (fireworks.found() && !fireworks.isHotbar()) {
                InventoryUtils.move().from(fireworks.slot()).toHotbar(elytraFly.replenishSlot.get() - 1);
            }
        }
        ...
        if (elytraFly.replace.get()) {
            ...
            if (chestStack.getItem() == Items.ELYTRA) {
                if (chestStack.getMaxDamage() - chestStack.getDamage() <= elytraFly.replaceDurability.get()) {
                    FindItemResult elytra = InventoryUtils.find(stack -> stack.getMaxDamage() - stack.getDamage() > elytraFly.replaceDurability.get() && stack.getItem() == Items.ELYTRA);
                    
                    InventoryUtils.move().from(elytra.slot()).toArmor(2);
                }
            }
        }
    }
    ...
    public void handleAutopilot() {
        ...
        if (elytraFly.useFireworks.get()) {
            if (ticksLeft <= 0) {
                ...
                FindItemResult itemResult = InventoryUtils.findInHotbar(Items.FIREWORK_ROCKET);
                if (!itemResult.found()) {
                    return;
                }
                ...
                if (itemResult.isOffhand()) {
                    ...
                    InventoryUtils.swap(itemResult.slot(), true);
    ...
    public void handleFallMultiplier() {
        if (velY < 0) {
            velY *= elytraFly.fallMultiplier.get();
        } else if (velY > 0) {
            velY = 0;
        }
    }

...
public class Scaffold extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        if (onlyOnClick.get() && !mc.options.useKey.isPressed()) {
            return;
        }
        ...
        FindItemResult result = InventoryUtils.findInHotbar(itemStack -> validItem(itemStack, bp));
    ...
    private boolean place(BlockPos bp) {
        FindItemResult item = InventoryUtils.findInHotbar(itemStack -> validItem(itemStack, bp));
        if (!item.found()) {
            return false;
        }

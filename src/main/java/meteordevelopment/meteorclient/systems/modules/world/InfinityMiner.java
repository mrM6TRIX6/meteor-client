...
import meteordevelopment.meteorclient.config.types.BlockListSetting;
...
public class InfinityMiner extends Module {
    ...
    private boolean findPickaxe() {
        ...
        FindItemResult bestPick = InventoryUtils.findInHotbar(pickaxePredicate);
        
        if (bestPick.isOffhand()) {
            InventoryUtils.shiftClick().fromOffhand().toHotbar(mc.player.getInventory().getSelectedSlot());
        } else if (bestPick.isHotbar()) {
            InventoryUtils.swap(bestPick.slot(), false);
        }
        
        return InventoryUtils.testInMainHand(pickaxePredicate);
    }

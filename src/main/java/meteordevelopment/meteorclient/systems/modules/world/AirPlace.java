...
public class AirPlace extends Module {
    ...
    public AirPlace() {
        super(Category.WORLD, "AirPlace", "Places a block where your crosshair is pointing at.");
    }
    
    @EventHandler
    private void onTick(TickEvent.Pre event) {
        if (!InventoryUtils.testInHands(this::placeable)) {
            return;
        }
    ...
    private void onInteractItem(InteractItemEvent event) {
        if (!(hitResult instanceof BlockHitResult bhr) || !placeable(mc.player.getStackInHand(event.hand))) {
            return;
        }
    ...
    private void onRender(Render3DEvent event) {
        if (!(hitResult instanceof BlockHitResult bhr)
            ...
            || !InventoryUtils.testInHands(this::placeable)
            || !render.get()) {
            return;
        }

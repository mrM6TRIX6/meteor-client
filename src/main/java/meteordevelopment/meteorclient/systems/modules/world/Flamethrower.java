...
public class Flamethrower extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (!entities.get().contains(entity.getType()) || !PlayerUtils.isWithin(entity, distance.get())) {
                continue;
            }
            ...
            if (!targetBabies.get() && entity instanceof LivingEntity && ((LivingEntity) entity).isBaby()) {
                continue;
            }
            
            FindItemResult findFlintAndSteel = InventoryUtils.findInHotbar(itemStack -> itemStack.getItem() == Items.FLINT_AND_STEEL && (!antiBreak.get() || itemStack.getDamage() < itemStack.getMaxDamage() - 1));
            if (!InventoryUtils.swap(findFlintAndSteel.slot(), true)) {
                return;
            }

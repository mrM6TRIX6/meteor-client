...
public class Notebot extends Module {
    ...
    public final Setting<Integer> checkNoteblocksAgainDelay = sgGeneral.add(new IntSetting.Builder()
        .name("CheckNoteblocksAgainDelay")
    ...
    private void setupNoteblocksMap() {
        ...
        for (var entry : incorrectNoteBlocks.entrySet()) {
            ...
            if (mode.get() == NotebotUtils.NotebotMode.EXACT_INSTRUMENTS) {
    ...
    /**
     * Set up a tune hits map which tells how many times player needs to hit noteblock to obtain desired note level
     */
        ...
        loadingSongFuture.whenComplete((song, ex) -> {
            if (ex == null) {
                ...
                if (ex instanceof CancellationException) {
                    ...
                    MeteorClient.LOGGER.error("An error occurred while loading song '{}'", FilenameUtils.getBaseName(file.getName()), ex);
                    onSongEnd();
                }
            }
        });
        ...
        for (int y = min; y < max; y++) {
            for (int x = min; x < max; x++) {
                for (int z = min; z < max; z++) {
                    ...
                    if (!mc.player.canInteractWithBlockAt(pos, 1)) {
                        continue;
                    }
    ...
    private void tuneBlocks() {
        ...
        while (iterator.hasNext()) {
            ...
            if (iterations == concurrentTuneBlocks.get()) {
                return;
            }
        }
    }

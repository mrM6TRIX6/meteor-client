...
public class BedAura extends Module {
    ...
    private final Setting<Color> sideColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new Color(15, 255, 211, 75))
        .build()
    );
    ...
    private final Setting<Color> lineColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new Color(15, 255, 211))
        .build()
    );
    ...
    private void onTick(TickEvent.Post event) {
        ...
        // Auto move
        if (autoMove.get()) {
            FindItemResult bed = InventoryUtils.find(itemStack -> itemStack.getItem() instanceof BedItem);
            
            if (bed.found() && bed.slot() != autoMoveSlot.get() - 1) {
                InventoryUtils.move().from(bed.slot()).toHotbar(autoMoveSlot.get() - 1);
            }
        }
    ...
    private BlockPos findPlace(PlayerEntity target) {
        if (!InventoryUtils.find(itemStack -> itemStack.getItem() instanceof BedItem).found()) {
            return null;
        }
        ...
        for (int index = 0; index < 3; index++) {
            ...
            for (CardinalDirection dir : CardinalDirection.values()) {
                if (strictDirection.get()
                    ...
                    && dir.toDirection().getOpposite() != mc.player.getHorizontalFacing()) {
                    continue;
                }
    ...
    private boolean placeBed(BlockPos pos) {
        ...
        FindItemResult bed = InventoryUtils.findInHotbar(itemStack -> itemStack.getItem() instanceof BedItem);
        if (bed.getHand() == null && !autoSwitch.get()) {
            return false;
        }

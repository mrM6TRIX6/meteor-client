...
public class AutoArmor extends Module {
    ...
    private void swap(int from, int armorSlotId) {
        InventoryUtils.move().from(from).toArmor(armorSlotId);
    ...
    private void moveToEmpty(int armorSlotId) {
        for (int i = 0; i < mc.player.getInventory().getMainStacks().size(); i++) {
            if (mc.player.getInventory().getStack(i).isEmpty()) {
                InventoryUtils.move().fromArmor(armorSlotId).to(i);
    ...
    private class ArmorPiece {
        ...
        public int getSortScore() {
            if (antiBreak.get() && durability <= 10) {
                return -1;
            }
            return bestScore;
        }
        ...
        public void apply() {
            ...
            if (bestScore > score) {
                swap(bestSlot, slot.getEntitySlotId());

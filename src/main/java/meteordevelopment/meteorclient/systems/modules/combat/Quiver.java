...
import meteordevelopment.meteorclient.config.types.StatusEffectListSetting;
...
public class Quiver extends Module {
    ...
    public void onActivate() {
        bow = InventoryUtils.find(Items.BOW);
        if (!shouldQuiver()) {
            return;
        }
        ...
        if (!bow.isMainHand()) {
            if (wasHotbar) {
                InventoryUtils.swap(bow.slot(), true);
            } else {
                InventoryUtils.move().from(mc.player.getInventory().getSelectedSlot()).to(prevSlot);
            }
        } else {
            wasMainhand = true;
        }
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (!charging) {
            InventoryUtils.move().from(arrowSlots.getFirst()).to(9);
            ...
            if (BowItem.getPullProgress(mc.player.getItemUseTime()) >= 0.12) {
                ...
                mc.interactionManager.stopUsingItem(mc.player);
                if (targetSlot != 9) {
                    InventoryUtils.move().from(9).to(targetSlot);
                }
    ...
    private boolean shouldQuiver() {
        if (!bow.found() || !bow.isHotbar() && !silentBow.get()) {
            if (chatInfo.get()) {
                error("Couldn't find a usable bow, disabling.");
            }
            toggle();
            return false;
        }
        ...
        if (!headIsOpen()) {
            if (chatInfo.get()) {
                error("Not enough space to quiver, disabling.");
            }
            toggle();
            return false;
        }
        ...
        if (EntityUtils.getTotalHealth(mc.player) < minHealth.get()) {
            if (chatInfo.get()) {
                error("Not enough health to quiver, disabling.");
            }
            toggle();
            return false;
        }
        ...
        if (onlyOnGround.get() && !mc.player.isOnGround()) {
            if (chatInfo.get()) {
                error("You are not on the ground, disabling.");
            }
            toggle();
            return false;
        }
    ...
    private boolean headIsOpen() {
        ...
        if (((AbstractBlockAccessor) pos1.getBlock()).meteor$isCollidable()) {
            return false;
        }
    ...
    private boolean hasEffect(StatusEffect effect) {
        for (StatusEffectInstance statusEffect : mc.player.getStatusEffects()) {
            if (statusEffect.getEffectType().value().equals(effect)) {
                return true;
            }
        }

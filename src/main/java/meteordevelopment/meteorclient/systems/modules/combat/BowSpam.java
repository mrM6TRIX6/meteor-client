...
public class BowSpam extends Module {
    ...
    private final Setting<Boolean> tridents = sgGeneral.add(new BoolSetting.Builder()
        .name("tridents")
        .description("Enable AutoBow functionality to tridents.")
        .defaultValue(false)
        .build()
    );
    ...
    public BowSpam() {
        super(Categories.Combat, "bow-spam", "Spams arrows and tridents.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        if (!mc.player.getAbilities().creativeMode && !InventoryUtils.find(itemStack -> itemStack.getItem() instanceof ArrowItem).found()) {
            return;
        }
        
        if (!onlyWhenHoldingRightClick.get() || mc.options.useKey.isPressed()) {
            boolean isBow = mc.player.getMainHandStack().getItem() == Items.BOW || (tridents.get() && mc.player.getMainHandStack().getItem() == Items.TRIDENT);
            if (!isBow && wasBow) {
                setPressed(false);
            }

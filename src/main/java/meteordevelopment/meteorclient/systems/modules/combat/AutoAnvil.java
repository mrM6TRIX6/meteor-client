...
public class AutoAnvil extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (placeButton.get()) {
            FindItemResult floorBlock = InventoryUtils.findInHotbar(itemStack -> Block.getBlockFromItem(itemStack.getItem()) instanceof AbstractPressurePlateBlock || Block.getBlockFromItem(itemStack.getItem()) instanceof ButtonBlock);
            BlockUtils.place(target.getBlockPos(), floorBlock, rotate.get(), 0, false);
        }
        
        if (++timer > delay.get()) {
            FindItemResult anvil = InventoryUtils.findInHotbar(itemStack -> Block.getBlockFromItem(itemStack.getItem()) instanceof AnvilBlock);
            if (!anvil.found()) {
                return;
            }

...
public class Surround extends Module {
    ...
    public void onActivate() {
        // Center on activate
        if (center.get() == Center.OnActivate) {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (!complete && center.get() == Center.Incomplete) {
            PlayerUtils.centerPlayer(true);
        }
    }
    ...
    private BlockType getBlockType(BlockPos pos) {
        ...
        else if (blockState.getBlock().getBlastResistance() >= 600) {
    ...
    private FindItemResult getInvBlock() {
        return InventoryUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem())));
    }

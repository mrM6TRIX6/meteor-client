...
public class ArrowDodge extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        for (Entity e : mc.world.getEntities()) {
            ...
            if (!allProjectiles.get() && !(projectile instanceof ArrowEntity)) {
                continue;
            }
            if (ignoreOwn.get()) {
    ...
    private boolean isValid(Vec3d velocity, boolean checkGround) {
        ...
        for (Vector3d pos : points) {
            ...
            if (projectilePos.isInRange(playerPos, distanceCheck.get())) {
                return false;
            }
            if (projectilePos.isInRange(headPos, distanceCheck.get())) {
                return false;
            }
        }

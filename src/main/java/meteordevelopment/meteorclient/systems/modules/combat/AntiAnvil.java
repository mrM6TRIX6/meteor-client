...
public class AntiAnvil extends Module {
    ...
    public AntiAnvil() {
        super(Category.COMBAT, "AntiAnvil", "Automatically prevents Auto Anvil by placing between you and the anvil.");
    }
    ...
    private void onTickPre(TickEvent.Pre event) {
        for (int i = 0; i <= mc.player.getBlockInteractionRange(); i++) {
            ...
            if (mc.world.getBlockState(pos).getBlock() == Blocks.ANVIL && mc.world.getBlockState(pos.down()).isAir()) {
                if (BlockUtils.place(pos.down(), InventoryUtils.findInHotbar(Items.OBSIDIAN), rotate.get(), 15, swing.get(), true)) {
                    break;
                }
            }
        }

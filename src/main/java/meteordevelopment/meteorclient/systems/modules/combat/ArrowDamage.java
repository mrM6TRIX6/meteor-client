...
import meteordevelopment.meteorclient.events.entity.player.StopUsingItemEvent;
...
public class ArrowDamage extends Module {
    ...
    private void onStopUsingItem(StopUsingItemEvent event) {
        ...
        for (int i = 0; i < packets.get() / 2; i++) {
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(x, y - 1e-10, z, true, mc.player.horizontalCollision));
            mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.PositionAndOnGround(x, y + 1e-10, z, false, mc.player.horizontalCollision));
        }
    }

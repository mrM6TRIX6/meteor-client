...
public class HoleFiller extends Module {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        FindItemResult block = InventoryUtils.findInHotbar(itemStack -> blocks.get().contains(Block.getBlockFromItem(itemStack.getItem())));
        if (!block.found()) {
            return;
        }
    ...
    private void onRender(Render3DEvent event) {
        ...
        for (Hole hole : holes) {
            ...
            for (int i = 0; i < holes.size(); i++) {
                if (!holes.get(i).equals(hole)) {
                    continue;
                }
                if (i < blocksPerTick.get()) {
                    isNext = true;
                }
            }
    ...
    private boolean validHole(BlockPos pos) {
        ...
        if (mc.player.getBlockPos().equals(testPos)) {
            return false;
        }
        if (distance(mc.player, testPos, false) > placeRange.get()) {
            return false;
        }
        if (mc.world.getBlockState(testPos).getBlock() == Blocks.COBWEB) {
            return false;
        }
        
        if (((AbstractBlockAccessor) mc.world.getBlockState(testPos).getBlock()).meteor$isCollidable()) {
            return false;
        }
        testPos.add(0, 1, 0);
        if (((AbstractBlockAccessor) mc.world.getBlockState(testPos).getBlock()).meteor$isCollidable()) {
        ...
        if (!mc.world.getOtherEntities(null, box, entity
            ...
            || entity instanceof EndCrystalEntity).isEmpty()) {
            return false;
        }

...
public class AutoGap extends Module {
    ...
    private void stopEating() {
        ...
        if (pauseAuras.get()) {
            for (Class<? extends Module> clazz : AURAS) {
                if (wasAura.contains(clazz)) {
                    Modules.get().get(clazz).enable();
                }
            }
        }
    ...
    private void changeSlot(int slot) {
        InventoryUtils.swap(slot, false);
        this.slot = slot;
    }
    ...
    private int findSlot() {
        for (int i = 0; i < 9; i++) {
            ...
            if (item == Items.ENCHANTED_GOLDEN_APPLE && allowEgap.get()) {
                return i;
            }
            
            // If gap was found and egap is not required we can return the current slot
            if (item == Items.GOLDEN_APPLE && !requiresEGap) {
                return i;
            }
        }

...
public class MapHud extends HudElement {
    ...
    @Nullable
    private MapIdComponent mapComponent;
    ...
    public void tick(HudRenderer renderer) {
        ...
        switch (mode.get()) {
            case SLOT_INDEX -> mapStack = mc.player.getInventory().getStack(slotIndex.get());
            case MAP_ID -> {
                FindItemResult mapResult = InventoryUtils.find(stack -> {
                    MapIdComponent mapIdComponent = stack.get(DataComponentTypes.MAP_ID);
                    return mapIdComponent != null && mapIdComponent.id() == mapId.get();
                });
                if (mapResult.found()) {
                    mapStack = mc.player.getInventory().getStack(mapResult.slot());
                }
            }
            case SIMPLE -> {
                FindItemResult mapResult = InventoryUtils.find(stack -> {

...
import meteordevelopment.meteorclient.config.types.EnchantmentListSetting;
    ...
    public void render(HudRenderer renderer) {
        renderer.post(() -> {
            ...
            if (isInEditor()) {
                ...
                playerEntity = TargetUtils.getPlayerTarget(range.get(), SortPriority.LOWEST_DISTANCE);
            }
            
            if (playerEntity == null && !isInEditor()) {
                return;
            }
            ...
            Color pingColor;
            if (ping <= 75) {
                pingColor = pingColor1.get();
            } else if (ping <= 200) {
                pingColor = pingColor2.get();
            } else {
                pingColor = pingColor3.get();
            }
            
            // Distance
            double dist = 0;
            if (!isInEditor()) {
                dist = Math.round(mc.player.distanceTo(playerEntity) * 100.0) / 100.0;
            }
            String distText = dist + "m";
            
            Color distColor;
            if (dist <= 10) {
                distColor = distColor1.get();
            } else if (dist <= 50) {
                distColor = distColor2.get();
            } else {
                distColor = distColor3.get();
            }

...
public interface IEntityRenderState {
     ...
     * <p>
     * The entity is only set after the render state is retrieved in EntityRenderDispatcher#render, so make sure not to
     * call this before that point (e.g. mixing into an updateRenderState method), otherwise the entity returned will
     * not be the same one that the render state is referring to.
     *
     * @return The entity that the render state refers to
     *
     * @see EntityRenderDispatcherMixin#render$getAndUpdateRenderState(EntityRenderState, Entity, double, double,
     * double, float, MatrixStack, VertexConsumerProvider, int, EntityRenderer)
     */

...
public class Renderer3D {
    ...
    private static @NotNull MeshBuilder getMeshBuilder() {
        return new MeshBuilder(MeteorRenderPipelines.WORLD_COLORED);
    }
    ...
    public void boxLines(double x1, double y1, double z1, double x2, double y2, double z2, Color color, int excludeDir) {
        ...
        if (excludeDir == 0) {
            ...
            // Bottom to top
            if (Dir.isNot(excludeDir, Dir.WEST) && Dir.isNot(excludeDir, Dir.NORTH)) {
                lines.line(blb, tlb);
            }
            if (Dir.isNot(excludeDir, Dir.WEST) && Dir.isNot(excludeDir, Dir.SOUTH)) {
                lines.line(blf, tlf);
            }
            if (Dir.isNot(excludeDir, Dir.EAST) && Dir.isNot(excludeDir, Dir.NORTH)) {
                lines.line(brb, trb);
            }
            if (Dir.isNot(excludeDir, Dir.EAST) && Dir.isNot(excludeDir, Dir.SOUTH)) {
                lines.line(brf, trf);
            }
            
            // Bottom loop
            if (Dir.isNot(excludeDir, Dir.WEST) && Dir.isNot(excludeDir, Dir.DOWN)) {
                lines.line(blb, blf);
            }
            if (Dir.isNot(excludeDir, Dir.EAST) && Dir.isNot(excludeDir, Dir.DOWN)) {
                lines.line(brb, brf);
            }
            if (Dir.isNot(excludeDir, Dir.NORTH) && Dir.isNot(excludeDir, Dir.DOWN)) {
                lines.line(blb, brb);
            }
            if (Dir.isNot(excludeDir, Dir.SOUTH) && Dir.isNot(excludeDir, Dir.DOWN)) {
                lines.line(blf, brf);
            }
            
            // Top loop
            if (Dir.isNot(excludeDir, Dir.WEST) && Dir.isNot(excludeDir, Dir.UP)) {
                lines.line(tlb, tlf);
            }
            if (Dir.isNot(excludeDir, Dir.EAST) && Dir.isNot(excludeDir, Dir.UP)) {
                lines.line(trb, trf);
            }
            if (Dir.isNot(excludeDir, Dir.NORTH) && Dir.isNot(excludeDir, Dir.UP)) {
                lines.line(tlb, trb);
            }
            if (Dir.isNot(excludeDir, Dir.SOUTH) && Dir.isNot(excludeDir, Dir.UP)) {
                lines.line(tlf, trf);
            }
        }
    }
    ...
    public void boxSides(double x1, double y1, double z1, double x2, double y2, double z2, Color color, int excludeDir) {
        ...
        if (excludeDir == 0) {
            ...
            // Bottom to top
            if (Dir.isNot(excludeDir, Dir.WEST)) {
                triangles.quad(blb, blf, tlf, tlb);
            }
            if (Dir.isNot(excludeDir, Dir.EAST)) {
                triangles.quad(brb, trb, trf, brf);
            }
            if (Dir.isNot(excludeDir, Dir.NORTH)) {
                triangles.quad(blb, tlb, trb, brb);
            }
            if (Dir.isNot(excludeDir, Dir.SOUTH)) {
                triangles.quad(blf, brf, trf, tlf);
            }
            
            // Bottom
            if (Dir.isNot(excludeDir, Dir.DOWN)) {
                triangles.quad(blb, brb, brf, blf);
            }
            
            // Top
            if (Dir.isNot(excludeDir, Dir.UP)) {
                triangles.quad(tlb, tlf, trf, trb);
            }
        }
    }

...
import meteordevelopment.meteorclient.gui.themes.meteor.widgets.input.WMeteorRangeSlider;
...
public class MeteorGuiTheme extends GuiTheme {
    ...
    public final Setting<Boolean> categoryIcons = sgGeneral.add(new BoolSetting.Builder()
        .name("category-icons")
        .description("Adds item icons to module Category.")
        .defaultValue(false)
        .build()
    );
    ...
    public final Setting<SettingColor> textHighlightColor = color(sgTextColors, "text-highlight", "Color of text highlighting.", new SettingColor(200, 200, 200, 50));
    ...
    @Override
    public WRangeSlider rangeSlider(Range value, int min, int max) {
        return w(new WMeteorRangeSlider(value, min, max));
    }
    ...
    @Override
    public WWidget module(Module module, String name) {
        return w(new WMeteorModule(module, name));
    }

...
public class DefaultSettingsWidgetFactory extends SettingsWidgetFactory {
    ...
    private void rangeW(WTable table, RangeSetting setting) {
        WRangeEdit edit = table.add(theme.rangeEdit(setting.get(), setting.min, setting.max, setting.sliderMin, setting.sliderMax, setting.noSlider)).expandX().widget();
    ...
    private <T extends Enum<T> & IDisplayName> void enumW(WTable table, EnumChoiceSetting<T> setting) {
        WDropdown<T> dropdown = table.add(theme.dropdown(setting.getChoices(), setting.get())).expandCellX().widget();
    ...
    private void gameModeListW(WTable table, GameModeListSetting setting) {
        selectW(table, setting, () -> mc.setScreen(new GameModeListSettingScreen(theme, setting)));
    }

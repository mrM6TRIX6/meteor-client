...
public abstract class WContainer extends WWidget {
    ...
    public boolean mouseReleased(Click click) {
        try {
            for (Cell<?> cell : cells) {
                if (propagateEvents(cell.widget()) && cell.widget().mouseReleased(click)) {
                    return true;
                }
            }
        } catch (ConcurrentModificationException ignored) {}
    ...
    public boolean keyPressed(KeyInput input) {
        try {
            for (Cell<?> cell : cells) {
                if (propagateEvents(cell.widget()) && cell.widget().keyPressed(input)) {
                    return true;
                }
            }
        } catch (ConcurrentModificationException ignored) {}
    ...
    public boolean keyRepeated(KeyInput input) {
        try {
            for (Cell<?> cell : cells) {
                if (propagateEvents(cell.widget()) && cell.widget().keyRepeated(input)) {
                    return true;
                }
            }
        } catch (ConcurrentModificationException ignored) {}
    ...
    public boolean charTyped(CharInput input) {
        try {
            for (Cell<?> cell : cells) {
                if (propagateEvents(cell.widget()) && cell.widget().charTyped(input)) {
                    return true;
                }
            }
        } catch (ConcurrentModificationException ignored) {}

...
public class DropCommand extends Command {
    ...
    private static final SimpleCommandExceptionType IN_SPECTATOR = new SimpleCommandExceptionType(Text.literal("Can't drop items while in spectator."));
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("hand")
            .executes(context ->
                drop(player -> player.dropSelectedItem(true))
            )
        );
        ...
        builder.then(literal("offhand")
            .executes(context ->
                drop(player -> InventoryUtils.drop().slotOffhand())
            )
        );
        ...
        builder.then(literal("hotbar")
            .executes(context ->
                drop(player -> {
                    for (int i = 0; i < 9; i++) {
                        InventoryUtils.drop().slotHotbar(i);
                    }
                })
            )
        );
            ...
            .executes(context ->
                drop(player -> {
                    for (int i = 9; i < player.getInventory().getMainStacks().size(); i++) {
                        InventoryUtils.drop().slotMain(i - 9);
                    }
                })
            )
        );
            ...
            .executes(context ->
                drop(player -> {
                    ...
                    if (!mc.player.getOffHandStack().isEmpty()) {
                        InventoryUtils.drop().slotOffhand();
                    }
                })
            )
        );
                ...
                drop(player -> {
                    for (EquipmentSlot equipmentSlot : AttributeModifierSlot.ARMOR) {
                        if (equipmentSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR) {
                            InventoryUtils.drop().slotArmor(equipmentSlot.getEntitySlotId());
                        }
                    }
                })
            )
